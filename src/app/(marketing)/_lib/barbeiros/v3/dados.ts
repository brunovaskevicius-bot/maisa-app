...
/** O rótulo do primeiro botão da página. Ação, não navegação: "Ativar" descreve o que
 *  acontece do outro lado; "Saiba mais" descreveria o que acontece com o mouse. */
export const CTA_ROTULO = "Ativar minha agenda";

/* ─────────────────────── a âncora, e para onde tudo aponta ─────────────────────── */

/** ⚠️ O DESTINO ÚNICO DE TODO CTA DESTA PÁGINA, escrito em um lugar só.
 *
 *  Até 10/08/2026 a página tinha UM botão (o da dobra) e ele apontava para
 *  `cfg.rotas.base` — ou seja, `/barbeiros/comecar`, OUTRA LP. Numa one-pager isso é
 *  vazamento: o único alvo da dobra levava a pessoa para fora antes de ela ver o
 *  preço, e a <Planos> ficava acessível só por rolagem. A própria page.tsx já dizia a
 *  regra que isso contrariava ("NÃO TEM NAV (…) uma one-pager não navega para nada").
 *
 *  Agora todo CTA aponta para a <Planos>, na mesma página. O `id` mora aqui e não
 *  digitado nos componentes porque são QUATRO consumidores (dobra, telas, duelo e a
 *  própria seção que recebe): trocar a âncora tem de ser uma linha, não uma caçada.
 *
 *  Se um dia existir uma PÁGINA de planos de verdade, é só trocar `HREF_PLANOS` por
 *  uma rota — nenhum componente sabe que hoje é âncora. */
export const ANCORA_PLANOS = "planos";
export const HREF_PLANOS = `#${ANCORA_PLANOS}`;

/** OS RÓTULOS DOS CTAs DE MEIO DE PÁGINA — um por seção, e de propósito DIFERENTES.
 *
 *  A tentação era carimbar "Ver planos" três vezes. Um CTA repetido vira moldura: o
 *  olho aprende a forma e para de ler. Cada rótulo aqui fecha o argumento da SEÇÃO em
 *  que ele está, e é por isso que eles moram junto do resto da copy em vez de virar
 ...
 *  para três momentos distintos da mesma descida. */
export const CTA_SECAO = {
  telas: "Quero isso no meu WhatsApp",
  duelo: "Escolher meu plano",
} as const;

/* ─────────────────────────── a frase do miolo ─────────────────────────── */

/** A manchete da dobra, no centro do anel de rostos (ver Orbita.tsx).
 *
 *  Chamava-se `FRASE_RODA` e morava no OCO de um anel de 64 rostos. A roda saiu em
 *  07/08/2026 e virou uma fila no pé; em 08/08 a fila virou uma ÓRBITA e o texto
 *  voltou a morar no meio dos rostos. Três arranjos, um referente: o "esses" sempre
 *  apontou para as pessoas que estão na tela junto com a frase.
 *
 ...
 *      "Todos esses podiam estar marcados com a {maisa}."
 */
/*  ⚠️ ERA UM ARRAY DE DUAS FRASES ATÉ 08/08/2026. A segunda ("Você não quer perder
 *  eles.") saiu a pedido, na mesma mexida em que a fila de rostos virou órbita: o
 *  texto encolheu porque as imagens passaram a ocupar a tela inteira.
 *
 *  O ARRAY FICA MESMO COM UM ELEMENTO SÓ, e não é inércia. Ele existe para impedir
 *  um bug específico que volta no minuto em que alguém acrescentar a segunda linha
 *  de novo: num parágrafo único o `text-wrap: balance` otimiza o COMPRIMENTO das
 *  linhas e não o sentido, e numa caixa de 287px ele quebrava em
 *      "Todos esses foram / marcados com a / maisa. Você não / quer perder eles."
 *  — separando o artigo do substantivo e colando o fim de uma frase no começo da
 *  outra. Uma entrada por frase resolve isso por construção: cada uma continua
 *  quebrando internamente conforme a viewport, mas nunca invade a vizinha.
 *  Ou seja: quem for escrever a linha 2 só precisa escrevê-la aqui. */
/* ══════════════════════════════════════════════════════════════════════════
 * ⚠️ MANCHETE NOVA EM 10/08/2026 — E ELA FECHA UM BURACO DE HONESTIDADE.
 *
 * A anterior era "Todos esses foram marcados com a {maisa}.", e o comentário acima
 * carregava um aviso de que ela AFIRMAVA UM FATO sobre gente de banco de imagem —
 * pessoas reais e identificáveis que nunca ouviram falar da marca. O aviso morreu
 * junto com a órbita: a dobra virou UMA FOTO de barbearia vazia, gerada, sem
 * ninguém dentro. Não há mais "esses" apontando para pessoas, então não há mais o
 * que afirmar sobre elas. É a primeira vez nesta página que uma troca de direção
 * de arte resolve um problema de copy em vez de criar um.
 *
 * O "esses" também deixou de ter referente, e é por isso que a manchete TINHA de
 * mudar junto com a imagem: as duas coisas eram uma só.
 *
 * ⚠️ MAS ENTRA UMA ALEGAÇÃO NOVA, E ELA É DE CAPACIDADE. O subtítulo diz "agenda
 * inteligente com IA". Este repositório não tem modelo de IA e não tem integração
 * com WhatsApp — está medido e escrito, com arquivo e linha, no cabeçalho da seção
 * das telas logo abaixo. A regra da casa que abre este arquivo ("se um dado não tem
 * origem verificável, ele não entra") foi escrita para NÚMEROS, e uma promessa de
 * produto não é um número; mas ela agora mora na primeira frase que a pessoa lê, que
 * é o lugar de maior peso da página. Se o motor existe no outro repositório da MAISA,
 * está tudo certo e isto é só rastreabilidade. Se não existe, a página está vendendo
 * o que não sabe fazer — e essa decisão é de quem assina a oferta, não deste arquivo.
 * ══════════════════════════════════════════════════════════════════════════ */
export const FRASE_DOBRA = ["Pare de perder dinheiro com agenda desorganizada."] as const;

/** O subtítulo, sob a manchete. Entrou junto com a foto: o pôster anterior vivia de
 *  UMA frase porque os rostos faziam o resto do trabalho. Sem eles, a manchete
 *  sozinha diz o problema e não diz a solução — o subtítulo é quem fecha o par. */
export const SUB_DOBRA =
  "Nossa agenda inteligente com IA marca, remarca e organiza seu dia todo.";
