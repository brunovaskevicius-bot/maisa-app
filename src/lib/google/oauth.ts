...
/** Erro que só se resolve reconectando: refresh token revogado/expirado. */
export class PrecisaReconectar extends Error {
  constructor(public motivo: string) {
    super(motivo);
    this.name = "PrecisaReconectar";
  }
}
...
export async function renovar(refreshToken: string): Promise<Tokens> {
  ...
  if (!r.ok) {
    ...
    throw new Error(`Falha ao renovar o acesso: ${d.error ?? r.status}`);
  }
...
export async function emailDaConta(accessToken: string): Promise<string> {
  ...
  if (!r.ok) throw new Error("Não foi possível ler a conta do Google.");
  const d = await r.json();
  if (!d.email) throw new Error("A conta do Google não devolveu e-mail.");
