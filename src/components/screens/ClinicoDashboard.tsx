...
export default function ClinicoDashboard() {
  // próxima sessão nos próximos 14 dias a partir de "hoje".
  const proximas = (() => {
    ...
    return out.sort((a, b) => a._t - b._t).slice(0, 1);
  })();
  ...
  // hero de vidro — um pouco maior; é a única peça da tela (calma e centrada).
  const glass: React.CSSProperties = {
    position: "relative", width: "100%", maxWidth: 620, textAlign: "center", padding: "60px 56px", borderRadius: 34,
  ...
  const kicker: React.CSSProperties = { marginTop: 24, fontSize: 13, fontWeight: 700, letterSpacing: ".14em", textTransform: "uppercase", color: "var(--primary-dark)" };
  const primaryBtn: React.CSSProperties = { display: "inline-flex", alignItems: "center", gap: 9, padding: "15px 24px", border: "none", borderRadius: 999, background: "var(--primary)", color: "#fff", fontWeight: 700, fontSize: 15, cursor: "pointer", fontFamily: "inherit" };
  ...
  return (
    ...
    {/* centrado vertical e horizontalmente: só a saudação + o card de vidro */}
    <div style={{ position: "relative", minHeight: "calc(100vh - 120px)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "48px 24px" }}>
      <div style={{ width: "100%", maxWidth: 620, display: "flex", flexDirection: "column", gap: 26 }}>

        {/* saudação calma acima do card */}
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 21, fontWeight: 800, color: "var(--ink)", letterSpacing: "-.01em" }}>Olá, Carla</div>
          <div style={{ fontSize: 14.5, color: "var(--muted)", marginTop: 3 }}>{dataHoje}</div>
        </div>

        {/* ───────── HERO de vidro — próxima sessão ───────── */}
        <div style={glass}>
          {next ? (
            <>
              <div style={{ width: 96, height: 96, borderRadius: "50%", background: avBg, color: avFg, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 34, margin: "0 auto", boxShadow: "0 8px 24px oklch(30% 0.03 60 / 0.12)" }}>{initials(next.nome)}</div>
              <div style={kicker}>Próximo na agenda</div>
              <h1 style={{ marginTop: 14, fontSize: 37, fontWeight: 700, lineHeight: 1.16, letterSpacing: "-.02em", color: "var(--ink)" }}>{next.servicoNome} com <span style={{ color: "var(--primary)" }}>{next.nome}</span></h1>
              <div style={{ marginTop: 18, display: "inline-flex", alignItems: "center", gap: 9, fontSize: 19, fontWeight: 600 }}>
                <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="var(--muted)" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="9" /><path d="M12 7v5l3 2" /></svg>
              ...
              <div style={{ marginTop: 34, display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
                ...
                <button onClick={verDetalhes} className="m-hov-bg m-press m-focus" style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "15px 24px", borderRadius: 999, border: "1px solid var(--border)", background: "rgba(255,255,255,0.6)", color: "var(--ink)", fontWeight: 700, fontSize: 15, cursor: "pointer", fontFamily: "inherit" }}>
            ...
            <div style={{ padding: "16px 0" }}>
              <div style={{ width: 96, height: 96, borderRadius: "50%", background: "var(--primary-soft)", color: "var(--primary-dark)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto" }}>
                <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="5" width="18" height="16" rx="3.2" /><path d="M3 9.5h18" /><path d="M8 3v3.4M16 3v3.4" /><circle cx="12" cy="14.8" r="1.7" /></svg>
              </div>
              <div style={kicker}>Próximo na agenda</div>
              <h1 style={{ marginTop: 14, fontSize: 32, fontWeight: 700, lineHeight: 1.2, letterSpacing: "-.02em", color: "var(--ink)" }}>Nenhuma sessão à vista</h1>
