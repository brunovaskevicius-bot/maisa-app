  ...
  return (
    <div className="lp3-z">
      ...
      <div className="lp3-z-status" aria-hidden="true">
        {/* A Dynamic Island. Não é enfeite: junto com a barrinha lá embaixo, é o par
            de formas que faz uma tela ser lida como iPhone antes de qualquer outra
            coisa — antes da fonte, antes dos ícones, antes do conteúdo. */}
        <span className="lp3-z-ilha" />
      ...
      {/* O indicador de home. Fora do fluxo e por cima da caixa de digitar, na faixa
          de 16 que o `padding-bottom` dela reserva — no aparelho real ele flutua
          sobre a interface, não empurra nada. */}
      <span className="lp3-z-barrinha" aria-hidden="true" />

      {/* ── O GLINT ──
          ⚠️ ERA UM `::after` DO `.lp3-z` E VIROU ELEMENTO DE VERDADE EM 08/08/2026.
          Não foi decisão de estilo, foi a única saída de uma regra do CSS: ele gira
          com `var(--d)`, e o `--d` passou a ser registrado com `inherits: false`
          (v3.css) para parar de invalidar os ~140 nós da conversa a cada quadro.
          Acontece que pseudo-elemento herda do elemento que o origina — e propriedade
          que NÃO herda simplesmente não chega nele: o `::after` passou a ler o
          `initial-value: 0` e o reflexo congelou de frente. Não dava para consertar
          escrevendo no `.lp3-z`, porque não existe estilo inline em pseudo-elemento.

          Como elemento real, a <Sincronia> escreve o `--d` DIRETO nele. Ele não tem
          filhos, então essa escrita invalida exatamente 1 nó — que era o objetivo da
          mudança toda. O desenho não mudou em nada: as regras são as mesmas, agora
          em `.lp3-z-brilho`.

          POR ÚLTIMO NO DOM porque ele é a camada de cima; o `z-index: 9` do CSS já
          garantia isso, mas empatar em z-index e depender da ordem do documento é a
          regra que esta página usa em todo lugar (ver a <Dobra>). */}
      <span className="lp3-z-brilho" aria-hidden="true" />
    </div>
  );
}
